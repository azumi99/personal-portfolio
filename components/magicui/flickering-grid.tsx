...
export const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const resolveColor = useCallback((colorValue: string | undefined): string => {
    ...
    if (!colorToResolve.startsWith("var(")) return colorToResolve;
  ...
  const setupCanvas = useCallback((canvas: HTMLCanvasElement, canvasWidth: number, canvasHeight: number) => {
  ...
  const drawGrid = useCallback((ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, cols: number, rows: number, squares: Float32Array, dpr: number) => {
